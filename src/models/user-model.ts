...
const UserSchema: mongoose.Schema<IUserDocument> = new mongoose.Schema<IUserDocument>({
    firstName: {type: String,
        ...
        maxlength: [50, 'First name must be at most 50 characters long'],
         ...
         confirmPassword: {
            ...
            maxlength: [100, 'Confirm Password must be at most 100 characters long'],
            select: false,
            trim: true
         },
         bio: {
         ...
         profileUrl: {
            type:String,
            trim: true,
            default: 'https://res.cloudinary.com/dzcmadjlq/image/upload/v1690991943/default-profile_xc6v4f.png'
         },
...
UserSchema.methods.comparepassword = async function (candidatePassword: string) : Promise<boolean> { 
    const isMatch = await bcrypt.compare(candidatePassword, this.password); //password of that specific user
    return isMatch; //Whenever I fetch a user from DB, that user can call comparePassword()
}

// (UserSchema as any).pre('save', async function(next: Function) {
//    if(Env.NODE_ENV === 'development'){
//       console.log("middleware logged before saving user" );
//    }
